...
namespace Bintronic_Inspect
{
    public partial class formMain : Form
    {
        ...
        public void EventAdd() //事件新增
        {
            ...
            //區塊2
            tbxInspector.LostFocus += tbxInspector_LostFocus;
        ...
        public void tbxInspector_LostFocus(object sender, EventArgs e) //檢測人員TextBox失去焦點
        {
            if (!string.IsNullOrEmpty(tbxInspector.Text.Trim()))
            {
                InspectMemberResponse inspectMemberResponse = getInspectMemberApi(tbxInspector.Text.Trim());
                if (inspectMemberResponse.success)
                {
                    if (!string.IsNullOrEmpty(inspectMemberResponse.data.Name.Trim())) tbxInspector.Text = inspectMemberResponse.data.Name.Trim();
                }
                else
                {
                    MessageBox.Show("員工資料抓取失敗，錯誤訊息：" + inspectMemberResponse.message);
                }
            }
        }
        ...
        {
            if (_intReciproca > 0) //如果_intReciproca 大於0則表示目前正在進行 SW的倒數，則直接跳到下一個檢測項目不要到數
            {
                _intReciproca = 0;
        ...
        public void btnLedOK_Click(object sender, EventArgs e) //LED檢測OK按鈕
        {
            ...
            changeLabelStyle(lblTestLedResult, "PASS");
        ...
        public void btnLedNG_Click(object sender, EventArgs e) //LED檢測NG按鈕
        {
            ...
            _IsAllSuccess = false; //有檢測失敗發生
            changeLabelStyle(lblTestLedResult, "FAILED");
        ...
        public void step1() //開始檢測
        {
            ...
            if (!_bolIsDebug) //由於電表被拆掉，因此在Debug期間暫時先關閉電壓電流讀取功能
            {
                if (!(readMeter())) //如果電壓電流溫度讀取失敗
                {
                    step0_1(); //保留結果停止檢測
                    lblDetectionProcessDescription.Text = "待測板連線錯誤，請將待測板放上治具，並治具USB接上電腦，開啟電源";
        ...
        public void step3() //檢測2，SW1全部為OFF
        {
            ...
            if (_strSw1Value != "000000") //如果測試不通過，則透過Timer重新跑10次，每次間隔1秒
            {
                _intTimerCount = 10; //重新執行10次
                tmrDetection.Interval = 1000; //1000毫秒間隔
                tmrDetection.Enabled = true; //啟用計時器
        ...
        public void step5_1() //判斷檢測1~4步驟是否有出現FAILED
        {
            ...
            {
                if (_detectionSend.data.sw1ONResult == "FAILED" || _detectionSend.data.sw1OFFResult == "FAILED") //只要SW1有其中一個失敗，最後結果就顯示失敗
                {
                    changeLabelStyle(lblTestSW1Result, "FAILED");
                }

                if (_detectionSend.data.sw2ONResult == "FAILED" || _detectionSend.data.sw2OFFResult == "FAILED") //只要SW2有其中一個失敗，最後結果就顯示失敗
                {
                    changeLabelStyle(lblTestSW2Result, "FAILED");
                }
        ...
        public void step8_1() //檢測485通訊P4與P5是否都為FAILED
        {
            ...
            {
                changeLabelStyle(lblTestModuleFirmwareVersionResult, "FAILED"); //顯示韌體版本
                sendDetection(); //傳送檢測紀錄
        ...
        {
            if (_bolIsDebug) return true; //如果是Debug模式
        ...
        public bool sendDetectionApi(out string strMessage) //實際透過API送出資料，回傳為是否成功的Bool，以及out錯誤訊息
        {
            ...
            try
            {
                ...
                string strApiPath = ConfigurationManager.AppSettings["ApiUrl_Create"]; //從App.config取得API網址
        ...
        public InspectMemberResponse getInspectMemberApi(string strEmpNum) //實際透過API取得檢測人員
        {
            InspectMemberResponse inspectMemberResponse = new InspectMemberResponse();
            try
            {
                //把要傳遞給API的員工編號包成物件
                InspectMemberSelectSend inspectMemberSelectSend = new InspectMemberSelectSend();
                inspectMemberSelectSend.data = new InspectMemberSelectSendData();
                inspectMemberSelectSend.data.EmpNum = strEmpNum;

                string strInspectMemberJson = JsonSerializer.Serialize(inspectMemberSelectSend); //把物件轉換成Json字串

                string strApiPath = ConfigurationManager.AppSettings["ApiUrl_readInspectMember"]; //從App.config取得API網址

                //建立 HttpClient
                HttpClient client = new HttpClient() { BaseAddress = new Uri(strApiPath) };

                var content = new StringContent(strInspectMemberJson, Encoding.UTF8, "application/json");
                var result = client.PostAsync(strApiPath, content).Result;

                inspectMemberResponse = JsonSerializer.Deserialize<InspectMemberResponse>(result.Content.ReadAsStringAsync().GetAwaiter().GetResult()); //單筆資料的JSON反序列化成物件

                return inspectMemberResponse;

                ////判斷API回傳的Success狀態，以此確定有沒有新增成功
                //if (inspectMemberResponse.success) //如果新增成功
                //{
                //    return true; //回傳字串給Controller
                //}
                //else //如果API回傳False
                //{
                //    strMessage = inspectMemberResponse.message;
                //    return false;
                //}
            }
            catch (Exception ex)
            {
                //strMessage = ex.ToString();
                inspectMemberResponse.message = ex.ToString();
                return inspectMemberResponse;
            }
        }
        ...
        {
            if (_bolIsDebug) //如果是Debug模式
            {
                switch (_intSteup)
                {
                    case 2:
                        if (_detectionSend.data.prodSerialNum.Substring(_detectionSend.data.prodSerialNum.Length - 1, 1) == "F") return "111110";
                        return "111111";
                        break;
                    case 3:
                        return "000000";
        ...
        private string readSw2() //讀SW2的2支PIN
        {
            if (_bolIsDebug) //如果是Debug模式
            {
                ...
                {
                    case 4:
                        if (_detectionSend.data.prodSerialNum.Substring(_detectionSend.data.prodSerialNum.Length - 1, 1) == "F") return "10";
                        return "11";
                        break;
                    case 5:
                        return "00";
